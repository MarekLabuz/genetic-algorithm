...
geneticLoop :: (Individual, Float) -> Population -> [Float] -> Int -> ReaderT Config IO (Individual, Float)
geneticLoop lowest population fitness generations = do
  ...
  if generations == 0 then return l else geneticLoop l newPopulation newFitness (generations - 1)
...
genetic name = do
  ...
  r <- runReaderT (geneticLoop low population fitness 1000) config
  -- return $ fst r
  return r
